module branch_control(
    ...
    output [15:0] PC_out,       // PC for branch
    output IF_Flush             // Signal to flush IF/ID pipeline (Branch IS taken)
);
    ...
    wire condition_met;
    PC_control branch_taker(.C(C), .I(offset), .F(F), .PC_in(PC_in), .PC_out(PC_B), .condition_met(condition_met));

    assign PC_out = (BR) ? read_data : PC_B;
    assign IF_Flush = condition_met & (BR | B);
