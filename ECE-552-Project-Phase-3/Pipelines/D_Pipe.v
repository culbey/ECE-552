...
dff ALU_A_reg[15:0] (.q(ALU_A_pipe), .d(ALU_A), .wen(en), .clk(clk), .rst(rst));
dff ALU_B_reg[15:0] (.q(ALU_B_pipe), .d(ALU_B), .wen(en), .clk(clk), .rst(rst));
dff Src2Data_reg[15:0] (.q(Src2Data_pipe), .d(Src2Data), .wen(en), .clk(clk), .rst(rst));
dff rt_reg[3:0] (.q(rt_pipe), .d(rt), .wen(en), .clk(clk), .rst(rst));
dff rs_reg[3:0] (.q(rs_pipe), .d(rs), .wen(en), .clk(clk), .rst(rst));
...
dff ALUOp_reg[2:0] (.q(ALUOp_pipe), .d(ALUOp), .wen(en), .clk(clk), .rst(rst));
dff WritePC_reg (.q(WritePC_pipe), .d(WritePC), .wen(en), .clk(clk), .rst(rst));
dff LLB_reg (.q(LLB_pipe), .d(LLB), .wen(en), .clk(clk), .rst(rst));
dff MemtoReg_reg (.q(MemtoReg_pipe), .d(MemtoReg), .wen(en), .clk(clk), .rst(rst));
dff LoadByte_reg (.q(LoadByte_pipe), .d(LoadByte), .wen(en), .clk(clk), .rst(rst));
dff WriteReg_reg (.q(WriteReg_pipe), .d(WriteReg), .wen(en), .clk(clk), .rst(rst));
dff NewN_reg (.q(NewN_pipe), .d(NewN), .wen(en), .clk(clk), .rst(rst));
dff NewZ_reg (.q(NewZ_pipe), .d(NewZ), .wen(en), .clk(clk), .rst(rst));
dff NewV_reg (.q(NewV_pipe), .d(NewV), .wen(en), .clk(clk), .rst(rst));
