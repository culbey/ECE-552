...
/* STATE TRANSITION */
localparam CHECK = 2'b0,
           MISS = 2'b1,

wire state, nxt_state; // 0 - IDLE, 1 - LOAD
assign nxt_state =  ((state == CHECK) & miss)       ?   MISS     :   // in load and we detect all chunks are loaded go to idle
...
assign even_hit = (state == CHECK) & (read | write) & (even_meta_out[7:2] == tag) & even_meta_out[1];
assign odd_hit = (state == CHECK) & (read | write) & (odd_meta_out[7:2] == tag) & odd_meta_out[1];

assign hit = (read | write) & (even_hit | odd_hit);
assign miss = ( ((read | write) & ~hit ) | (state == MISS));

assign write_meta_even = hit;
assign write_meta_odd = odd_hit & write;

assign even_meta_in =   (write & even_hit)     ?   {tag, 1'b1, 1'b1}               :       // if the hit was even then ensure to update LRU bit and write the new tag
                        (read & even_hit)      ?   {even_meta_out[7:1], 1'b1}      :      // if the hit was even then update LRU and keep everything else the same
                        {even_meta_out[7:1], 1'b0};                                     // if the hit was odd then update LRU and keep everything else the same

assign odd_meta_in =    {tag, 1'b1, 1'b0};


assign write_data_even = (write & even_hit) | ((state == MISS) & mem_data_valid & ~even_meta_out[0]);
assign write_data_odd = (write & odd_hit) | ((state == MISS) & mem_data_valid & even_meta_out[0]);

assign even_data_in = write_data;
assign odd_data_in = write data;
